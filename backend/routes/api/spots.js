...
router.get('/', async (req, res, next) => {
  try {
    ...
    if (size === undefined || isNaN(size) || size < 1 || size > 20) {
      errors.size = 'Size must be between 1 and 20';
    }

    if (req.query.minLat === undefined || isNaN(parseFloat(req.query.minLat))) {
      errors.minLat = 'Minimum latitude is invalid';
    }

    if (req.query.maxLat === undefined || isNaN(parseFloat(req.query.maxLat))) {
      errors.maxLat = 'Maximum latitude is invalid';
    }

    if (req.query.minLng === undefined || isNaN(parseFloat(req.query.minLng))) {
      errors.minLng = 'Minimum longitude is invalid';
    }

    if (req.query.maxLng === undefined || isNaN(parseFloat(req.query.maxLng))) {
      errors.maxLng = 'Maximum longitude is invalid';
    }

    if (req.query.minPrice === undefined || (isNaN(parseFloat(req.query.minPrice)) || parseFloat(req.query.minPrice) < 0)) {
      errors.minPrice = 'Minimum price must be greater than or equal to 0';
    }

    if (req.query.maxPrice === undefined || (isNaN(parseFloat(req.query.maxPrice)) || parseFloat(req.query.maxPrice) < 0)) {
